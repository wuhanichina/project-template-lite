...
引言写作先面向小同行专家判定最小可辨识研究对象，再判定研究领域、具体工程对象和研究方向。`ProjectName_note.md` 的引言草稿直接在研究笔记内完成，不调用 `$powerlit-power-systems-paper-writing` 或 PowerLit；PowerLit 默认只用于 `01_IDEA/figure_plan.md` 的图表和结果证据计划。第一段从工程背景和重要性逐层铺垫并点出研究对象；第二段评述已有方法及其在本文对象上的具体缺口；第三段从物理机理和数学模型解释问题尚未闭合的原因；第四段说明本文发现或验证了什么判断、从什么角度建立什么模型、解决什么实际工程问题；第五段按需要承接验证设计；第六段按发现或验证的判断、提出的方法或模型、解决的工程问题、可验证输出和证据来源写正向贡献。
