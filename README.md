...
- 2026-07-05：增加 Antigravity / Gemini 兼容入口，新增 `GEMINI.md`、`.agents/rules/project-template-lite.md` 和 `.agent/rules/project-template-lite.md`，并明确 `AGENTS.md` 与 `.cursor/rules/` 为共享规则源。
...
| AI 助手规则 | `AGENTS.md`、`GEMINI.md`、`CLAUDE.md`、`.cursor/rules/`、`.agents/rules/`、`.agent/rules/` | 约束代码组织、出图出口、证据同步、写作约定和不同 agent 工具入口 |
...
├── .cursor/                  # 共享细则，Cursor/Codex/Claude/Antigravity 共同引用
├── .agents/                  # Antigravity 当前 workspace rules 入口
└── .agent/                   # Antigravity 旧路径兼容入口
```
...
| AI 入口文件 | `AGENTS.md`、`GEMINI.md`、`CLAUDE.md`、`.agents/rules/project-template-lite.md`、`.agent/rules/project-template-lite.md` | 给 Codex、Gemini CLI、Antigravity 和 Claude Code 提供入口，并指向共享规则源 |
...
`AGENTS.md` 是本模板的通用 agent 规则入口，适用于 Codex 和其他读取 `AGENTS.md` 的 coding agent。`GEMINI.md` 是 Gemini CLI 和 Antigravity 相关上下文的根入口；`CLAUDE.md` 是 Claude Code 入口。三者都保持为薄入口，详细执行规则统一放在 `.cursor/rules/`。

Antigravity workspace rules 同步放置两份薄入口：

- `.agents/rules/project-template-lite.md`：当前 Antigravity workspace rules 入口。
- `.agent/rules/project-template-lite.md`：旧路径兼容入口。

复制模板或更新规则时，保持 `AGENTS.md`、`GEMINI.md`、`CLAUDE.md`、`.agents/rules/`、`.agent/rules/` 和 `.cursor/rules/` 同步提交。不要在这些入口文件里复制长规则正文；入口文件只负责把不同工具导向同一套共享规则。
...
引言写作先面向小同行专家判定最小可辨识研究对象，再判定研究领域、具体工程对象和研究方向。第一段从工程背景和重要性逐层铺垫并点出研究对象；第二段评述已有方法及其在本文对象上的具体缺口；第三段从物理机理和数学模型解释问题尚未闭合的原因；第四段说明本文从什么角度建立什么模型并解决什么问题；第五段按需要承接验证设计；第六段写正向贡献。

文字风格要求物理概念清晰、数学逻辑简练。优先使用节点电压、线路功率、潮流约束、源荷过程、运行场景、概率分布、越限事件等电力系统对象和物理量；过于数学化或编程化的词只在确有严格定义时使用。

优先使用“当前证据支持”“该判断仍需验证”“该路线已归档”“该结论目前仅在某算例中观察到”等表达，让读者看到证据强度。

AI 生成的研究笔记、论文草稿、审稿回复、图注、README 和代码注释应通过 `.cursor/rules/03-public-note-and-paper-skill-bridge.mdc` 中的写作风格检查，并用 `.cursor/rules/lexicon.md` 做最终词表筛查。文本应直接陈述研究对象、工程痛点、模型关系、工程场景、指标和证据状态。创新点应写成正向技术句，先说明本文建立了什么、解决了什么、输出了什么，再给出证据来源和后续实验。
...
| AI 助手规则 | `AGENTS.md`、`GEMINI.md`、`CLAUDE.md`、`.agents/rules/`、`.agent/rules/` 和 `.cursor/rules/` 随模板变化同步提交 |
...
正式研究材料和 AI 助手执行规则分开放置。`README.md`、`ProjectName_note.md`、`ProjectName_changelog.md` 和 `01_IDEA/*` 面向人类读者，呈现研究内容、证据状态、版本摘要和交接路径；`AGENTS.md`、`GEMINI.md`、`CLAUDE.md`、`.agents/rules/`、`.agent/rules/` 和 `.cursor/rules/` 面向 AI 助手，保存入口、执行约束、代码组织规则和写作检查规则。
