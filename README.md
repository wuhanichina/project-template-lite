...
├── lexicon.md                # AI 写作词表：套话、空泛词和范围过强表述筛查
...
`+ProjectName_utils/` 放附属代码，即不定义贡献但项目会复用的支撑工具，例如目录创建、summary 输出、随机种子、缓存读写和绘图导出。绘图工具放在 `+ProjectName_utils/+plotting/`。
...
| AI 写作词表 | `lexicon.md` | 统一检查中英文 AI 生成文本中的套话、空泛词和范围过强表述 |
...
AI 生成的研究笔记、论文草稿、审稿回复、图注、README 和代码注释应通过 `.cursor/rules/03-public-note-and-paper-skill-bridge.mdc` 中的写作风格检查，并用 `lexicon.md` 做最终词表筛查。文本应直接陈述技术对象、机制、范围、指标和证据状态。
...
每个入口负责创建自己的结果目录、缓存目录和图表目录。核心算法放入 `+ProjectName_core/`，入口脚本保持为清晰的 case orchestration。
