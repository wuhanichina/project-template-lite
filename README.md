...
[📝 版本更新记录](#版本更新记录) · [📌 使用场景](#使用场景) · [🚀 三分钟上手](#三分钟上手) · [🧩 模板管理哪些材料](#模板管理哪些材料) · [🛠️ 常见任务怎么做](#常见任务怎么做) · [🔁 推荐使用流程](#推荐使用流程) · [📂 目录结构](#目录结构) · [🤖 AI 助手兼容](#ai-助手兼容) · [🧪 运行环境与 smoke test](#运行环境与-smoke-test) · [✅ 提交前检查清单](#提交前检查清单) · [🔒 使用约定](#使用约定)

---

## 版本更新记录

仅保留最近 10 个日期版本；同一日期的多项修改合并为一条。

- 2026-07-12：在 `.cursor/` 中统一版本化 PowerLit handoff contract 与 AI 规则，新增受版本控制的 paper profile、创新/生命周期字段和按创新类型选择的对外叙事路线，联通 Figure-first 计划、run/figure manifest、投稿一致性报告与返修工件，并扩展 MATLAB smoke。
- 2026-07-10：新增证据约束的 `ProjectName_blog.md`，修复正式图事务式导出、并发锁、异常回滚与 claim 门禁，补齐失败运行 manifest、递归输入 SHA-256 指纹和跨平台重命名，并增加完整 smoke suite 与 MATLAB CI。
- 2026-07-07：要求项目引言和创新点先提炼理论意义与工程价值，指标、算例和图表承担证据支撑作用。
- 2026-07-06：集中更新写作契约，补充创新层级记录、引言草稿位置、PowerLit 使用边界、词表硬禁用/慎用规则和领域通用术语要求，并移除 `.agents/`、`.agent/` 兼容路径，改由 `GEMINI.md` 引导 Gemini CLI / Antigravity 读取共享规则。
- 2026-07-05：增加 Gemini CLI / Antigravity 兼容入口 `GEMINI.md`，明确 `AGENTS.md` 与 `.cursor/rules/` 为共享规则源，并强化最小可辨识研究对象、正向创新写作、工程痛点、模型关系、可验证输出和证据来源的组织方式。
- 2026-07-04：新增 `ProjectName_changelog.md` 作为研究内容 changelog，将 `ProjectName_note.md` 的长更新记录移出，并要求研究内容版本按日期倒序登记。
- 2026-07-03：补齐算例级 `run_manifest.json`、分叉探索记录、case 注册表和图表计划填写标准，统一 run manifest 与 trace schema，去掉 `ProjectName.m` 的 `cd` 副作用，并将 04 图表与指标规划规则纳入 agent 入口。
...
2. 把 `ProjectName` 替换为项目简称：
   ...
   - `+ProjectName_utils/`
   - `+ProjectName_sota/`
3. 复制 `02_PAPER/config/paper-profile.example.yaml` 为受版本控制的
   `paper-profile.yaml`，确认目标期刊、最小研究对象、创新四轴、叙事、
   生命周期和证据边界。
4. 填 `01_IDEA/story.md`，判定最小可辨识研究对象、研究领域、具体工程对象和研究方向，写清楚工程痛点、物理原因和科学问题。
5. 再填 `01_IDEA/claims.md`，用正向技术句把可验证结论绑定到创新轴、幅度、算例契约、生命周期、证据编号和验证标准。
6. 在 `01_IDEA/symbols.md` 登记符号、单位、维度和代码变量映射。
7. 把完整复现输入放入 `data/`，包括原始数据、算例文件、参数表和必要外部输入。
8. 先在 `01_IDEA/figure_plan.md` 登记 Figure-first 证据问题、预期趋势、关键点、待检验机理、优势判据和边界，再运行正式导出。
9. 把正式计算写进根目录的 `ProjectName_case*.m`，把可复用函数放入对应 MATLAB 包目录。
10. 在 MATLAB 中进入项目根目录，运行一个正式算例：
...
11. 有正式结果后，同步更新：
   ...
   - 对应的 `result/<算例名>/` 路径
12. 提交前运行 `02_PAPER/submission_consistency.md` 所记录的 PowerLit
    一致性终检，并临时移走或清空 `cache/`，确认 `ProjectName` 或正式
    `ProjectName_case*.m` 能只依赖代码和 `data/` 重新开始。
...
```powershell
$New = "Abc"   # 改成你的项目简称
$TextExtensions = @(".m", ".md", ".mdc", ".yaml", ".yml", ".json")
$Utf8Strict = New-Object System.Text.UTF8Encoding($false, $true)
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)

if ($New -cnotmatch '^[A-Za-z][A-Za-z0-9_]*$') {
  throw "项目简称必须以字母开头，且只能包含字母、数字和下划线。"
}

# 1. 按 UTF-8 读取和写回文本；显式编码可兼容 Windows PowerShell 5.1
Get-ChildItem -LiteralPath . -Recurse -File -Force |
  ...
  ForEach-Object {
    ...
    $Text = $Text.Replace("ProjectName", $New)
    [System.IO.File]::WriteAllText($_.FullName, $Text, $Utf8NoBom)
  }

# 2. 重命名含 ProjectName 的文件和目录（自底向上，先深后浅）
$RenameTargets = @(Get-ChildItem -LiteralPath . -Recurse -Force |
  Where-Object {
    $_.FullName -notmatch '[\\/]\.git[\\/]' -and
...
$RenameTargets | ForEach-Object {
  $OldPath = $_
  $NewName = [System.IO.Path]::GetFileName($OldPath).Replace("ProjectName", $New)
  Rename-Item -LiteralPath $OldPath -NewName $NewName
}

# 3. 检查是否还有残留
$TextFiles = @(Get-ChildItem -LiteralPath . -Recurse -File -Force |
  Where-Object {
    $_.FullName -notmatch '[\\/]\.git[\\/]' -and
    $TextExtensions -contains $_.Extension.ToLowerInvariant()
  } | Select-Object -ExpandProperty FullName)
Select-String -Path $TextFiles -Pattern 'ProjectName' -List
```

macOS（在项目根目录用 Bash 运行）：

```bash
NEW=Abc   # 改成你的项目简称
[[ "$NEW" =~ ^[A-Za-z][A-Za-z0-9_]*$ ]] || { echo "项目简称格式无效"; exit 1; }

# 1. 替换文本内容；BSD sed 的 -i 需要空备份后缀
find . -type f \( -name '*.m' -o -name '*.md' -o -name '*.mdc' -o \
  -name '*.yaml' -o -name '*.yml' -o -name '*.json' \) \
  -not -path './.git/*' -exec sed -i '' "s/ProjectName/${NEW}/g" {} +

# 2. 重命名含 ProjectName 的文件和目录（自底向上）
find . -depth -name '*ProjectName*' -not -path './.git/*' -print0 | \
...
Linux（在项目根目录用 Bash 运行）：

```bash
NEW=Abc   # 改成你的项目简称
[[ "$NEW" =~ ^[A-Za-z][A-Za-z0-9_]*$ ]] || { echo "项目简称格式无效"; exit 1; }

# 1. 替换文本内容；GNU sed 直接使用 -i
find . -type f \( -name '*.m' -o -name '*.md' -o -name '*.mdc' -o \
  -name '*.yaml' -o -name '*.yml' -o -name '*.json' \) \
  -not -path './.git/*' -exec sed -i "s/ProjectName/${NEW}/g" {} +
...
| 对外叙事参考 | `ProjectName_blog.md` | 按创新轴、技术对象和叙事路线组织工程说明，保留适用范围、证据来源和证据状态，不作为证据索引或论文正文模板 |
...
| AI 助手规则 | `AGENTS.md`、`GEMINI.md`、`CLAUDE.md`、`.cursor/` | 集中保存代码、出图、证据、写作规则和 handoff contract，并提供不同 agent 工具入口 |
...
| 写对外介绍 | `ProjectName_blog.md`、`ProjectName_note.md` | `ProjectName_blog.md`，必要时同步 `story.md`、`claims.md` 或 `ProjectName_changelog.md` | blog 使用已确认的创新轴、技术对象和叙事路线，项目判断可追到范围、证据来源和证据状态，且未把叙事文案当成论文证据 |
...
1. 从 `paper-profile.yaml` 和 `story.md` 确认最小可辨识研究对象、主创新轴、技术对象、叙事路线、创新幅度、生命周期和证据边界。
2. 在 `ProjectName_blog.md` 用“起、承、转、合、升”组织从工程实际问题到关键关系、物理机理、现有不足、本文处理和工程含义的总体推进；主创新轴决定故事中心，技术对象决定解释层级，`narrativeArc` 决定各环节的篇幅重心。
3. 项目特有判断必须能追到 `claims.md`、`derivation.md`、`evidence_map.md`、登记结果或已核查文献。作者经验和工程直觉只能形成 `仅为当前假设`、`尚未验证` 或 `文献待核查` 的问题，不能单独支撑强结论。
4. blog 只提供对外叙事顺序，不提供论文证据或可直接粘贴的正文。Introduction 和 Contributions 必须从研究笔记、结论、推导、登记结果与参考文献重新提炼和改写。
5. 当 `story.md`、`claims.md`、`evidence_map.md` 或正式结果改变了对外叙事时，同步检查 `ProjectName_blog.md` 是否需要改写。
...
├── ProjectName_blog.md       # 对外叙事参考：范围、证据和证据状态受研究档案约束
...
└── .cursor/                  # 共享 AI 规则与 handoff contract，各类 agent 共同引用
```
...
`ProjectName_blog.md` 是证据约束的对外叙事参考。它推荐用“起、承、转、合、升”完成从工程实际问题到关键关系、物理机理、现有不足、本文处理和工程含义的总体推进。主创新轴决定文章中心，技术对象决定解释物理因果、模型映射、计算性质、状态响应、成立条件、观测能力或决策关系中的哪一层，`narrativeArc` 决定各环节的篇幅重心。它不是证据索引，也不是论文正文模板；每项项目判断都要能追到适用范围、证据来源和证据状态。
...
| 类型 | 文件 | 作用 |
| --- | --- | --- |
| 证据承载材料 | `ProjectName_note.md`、`ProjectName_changelog.md`、`01_IDEA/`、登记结果 | 记录问题、结论、推导、证据、版本摘要和路线演化，支撑复核与论文提炼 |
| 对外叙事参考 | `ProjectName_blog.md` | 在证据边界内组织外部说明，不替代结论、推导、结果、文献或论文正文 |
...
| AI 执行规则与合同 | `.cursor/rules/`、`.cursor/contracts/` | 集中约束 AI 助手的代码组织、出图出口、证据同步、写作约定和 Skill 交接字段 |
...
`AGENTS.md` 是本模板的通用 agent 规则入口，适用于 Codex 和其他读取 `AGENTS.md` 的 coding agent。`GEMINI.md` 是 Gemini CLI 和 Antigravity 相关上下文的根入口，Antigravity 通过该文件读取 `AGENTS.md` 和 `.cursor/`；`CLAUDE.md` 是 Claude Code 入口。三者都保持为薄入口，详细执行规则和 handoff contract 统一放在 `.cursor/`。

本模板不维护 `.agents/`、`.agent/` 或 `.codex/` 目录。复制模板或更新规则时，保持 `AGENTS.md`、`GEMINI.md`、`CLAUDE.md` 和 `.cursor/` 同步提交。不要在这些入口文件里复制长规则正文；入口文件只负责把不同工具导向同一套共享规则和合同。
...
推荐写法是中性、客观、可验证、可追溯。写作首先判定最小可辨识研究对象，创新点使用正向技术句，写清具体工程对象、工程痛点、理论意义、工程价值、模型关系、可验证输出和证据来源；证据不足和未来工作放入对应字段，贡献句直接呈现本文建立的模型、解决的问题和产生的输出。

实际创新按层级记录：首先写本文发现了什么机制、规律或关系，或验证了什么研究猜想；然后写本文提出了什么模型、约束、流程或方法；最后写该方法解决了什么实际工程问题，并给出推导、算例、指标或图表证据。

项目引言和创新点应站在理论意义和工程价值上组织材料。理论意义可以来自机理认识、模型关系、评价口径或可解释计算的推进；工程价值可以来自规划、运行、调度、定价、控制、风险评估或工程决策的作用。指标、算例和图表用于支撑这些判断。
...
引言写作先面向小同行专家判定最小可辨识研究对象，再判定研究领域、具体工程对象和研究方向。`ProjectName_note.md` 的引言草稿直接在研究笔记内完成，不调用 `$powerlit-power-systems-paper-writing` 或 PowerLit；PowerLit 默认只用于 `01_IDEA/figure_plan.md` 的图表和结果证据计划。第一段从工程背景和重要性逐层铺垫并点出研究对象；第二段评述已有方法及其在本文对象上的具体缺口；第三段从物理机理和数学模型解释问题尚未闭合的原因，并提炼理论意义；第四段说明本文发现或验证了什么判断、从什么角度建立什么模型、形成什么理论意义、服务什么工程价值；第五段按需要承接验证设计；第六段按发现或验证的判断、提出的方法或模型、理论意义、工程价值、解决的工程问题、可验证输出和证据来源写正向贡献。
...
AI 生成的研究笔记、论文草稿、审稿回复、图注、README 和代码注释应通过 `.cursor/rules/03-public-note-and-paper-skill-bridge.mdc` 中的写作风格检查，并用 `.cursor/rules/lexicon.md` 做最终词表筛查。文本应直接陈述研究对象、工程痛点、理论意义、工程价值、模型关系、工程场景和证据状态。创新点应写成正向技术句，先说明本文建立了什么、形成什么理论意义、服务什么工程价值、解决了什么，再给出证据来源和后续实验；正文方法名称应能被小同行直接理解。
...
模板以 MATLAB R2020a 作为目标最低兼容版本。正式图导出使用 R2020a 引入的 `exportgraphics`，其余模板代码还使用 `arguments`、string、table、JSON 和标准 MATLAB JVM 接口。三项 smoke test 依赖基础 MATLAB、标准 JVM、可用的本地图形环境和命令行可调用的 Git，不需要 CVX、MOSEK、Gurobi 或项目数据；具体研究模型可以在此基础上声明额外 toolbox、求解器和数据依赖。当前本地终检使用 R2025b，R2020a 兼容性由语法/API 边界和 R2021a CI 近端覆盖约束，不能等同于 R2020a 实机验证。
...
测试检查默认 IEEE 图形 profile、FIG/PNG/SVG/PDF/CSV 事务提交与异常回滚、并发锁、manifest 当前登记、正式 claim 与证据角色门禁、override 原因、递归输入 SHA-256 指纹、Git porcelain 状态，以及两个 case 入口的成功与失败 manifest。命令以非零状态退出时，不应把当前模板状态视为已通过验证。

`.github/workflows/matlab-smoke.yml` 在 push、pull request 和手动触发时运行同一套 smoke suite。官方 MATLAB Actions 的托管 runner 从 R2021a 起可用，因此 CI 固定 R2021a，而本地目标最低代码版本仍为 R2020a。公开仓库由 MATLAB Actions 自动许可；私有仓库需把 MATLAB batch licensing token 保存为仓库 secret `MATLAB_BATCH_TOKEN`，workflow 会将其映射到官方要求的 `MLM_LICENSE_TOKEN` 环境变量。
...
| 运行环境 | 目标最低兼容 MATLAB R2020a；项目额外 toolbox、求解器和数据依赖已在入口或 README 声明，并记录实际验证 release |
| smoke test | `tests/run_template_smoke_tests.m` 已通过，图形 profile、事务提交/回滚、并发锁、claim 门禁、运行 manifest 和输入指纹均符合合同 |
...
| 对外介绍 | `ProjectName_blog.md` 已替换项目名并使用已确认的创新轴、技术对象和叙事路线；项目判断可追到范围、证据来源和证据状态，且没有被当成论文证据或可直接粘贴的正文 |
...
| AI 助手规则 | `AGENTS.md`、`GEMINI.md`、`CLAUDE.md` 和 `.cursor/` 随模板变化同步提交，根目录不保留 `.codex/` 或独立 `contracts/` |
...
- 模板入口和 MATLAB 包目录。
- `.cursor/` 中的共享 AI 规则和 handoff contract。
...
正式研究材料和 AI 助手执行规则分开放置。`README.md`、`ProjectName_note.md`、`ProjectName_changelog.md` 和 `01_IDEA/*` 面向人类读者，呈现研究内容、证据状态、版本摘要和交接路径；`AGENTS.md`、`GEMINI.md`、`CLAUDE.md` 和 `.cursor/` 面向 AI 助手，保存入口、执行约束、代码组织规则、写作检查规则和 handoff contract。
