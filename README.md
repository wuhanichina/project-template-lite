...
| 增加研究问题 | `01_IDEA/story.md` | `story.md`、`claims.md`、`ProjectName_note.md`、`ProjectName_changelog.md` | 新问题有结论记录、证据状态、版本摘要和后续验证路径 |
...
| 投稿前复核 | `01_IDEA/evidence_map.md` | `ProjectName_note.md`、`ProjectName_changelog.md`、结果目录、稿件材料 | 证据、模型、指标和结论边界一致 |
...
5. 审稿意见改变结论边界时，先更新 `claims.md`、`ProjectName_note.md` 和 `ProjectName_changelog.md`。
...
`ProjectName_note.md` 开头保留当前版本日期和一句话摘要；完整版本历史写入 `ProjectName_changelog.md`。`ProjectName_changelog.md` 使用 `YYYY-MM-DD` 日期版本，按时间倒序排列，只记录会改变研究问题、路线、模型、假设、实验协议、结果解释、证据状态、图表计划、论文边界或交接判断的修改。
...
正式研究材料和 AI 助手执行规则分开放置。`README.md`、`ProjectName_note.md`、`ProjectName_changelog.md` 和 `01_IDEA/*` 面向人类读者，呈现研究内容、证据状态、版本摘要和交接路径；`.cursor/rules/` 面向 AI 助手，保存执行约束、代码组织规则和写作检查规则。
