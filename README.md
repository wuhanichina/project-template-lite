...
- 若遗留复现确实需要平台限定工具，应隔离成可选 adapter，并在入口、证据图或 README 中写明它不是主流程冷启动依赖。
...
公共 Git 可以因为体积或隐私暂不跟踪全部 `data/` 文件，但正式项目提交包必须包含完整 `data/` 输入。若某个必要输入尚不能随项目提供，应在 `data/README.md` 和 `01_IDEA/evidence_map.md` 中标记为未闭合复现边界，而不是用 `cache/` 代替。
